!function (d) {
		...
		, m = {
			...
			form: function (n, r) {
				...
				if ("string" == typeof n && (n = document.querySelector(n)),
					!((n = d.jQuery && n instanceof d.jQuery ? n[0] : n) instanceof d.HTMLFormElement))
				...
				n.classList.add(e),
					n.onsubmit = function (e) {
						...
						s && (n.classList.add(i),
							m._showSubmitting(n),
							m._send(t.key, t.url, o, e, r, function (e, t) {
								s && m._hideSubmitting(n),
									n.classList.remove(i),
									n.classList.add(e ? a : c),
									!1 === (!r.onResponse || r.onResponse(e, t)) || e || m._showSuccess(n, u),
									e ? console.error(e.message || e) : n.reset(),
									setTimeout(function () {
										l.stop();
									}, 200);
							}));
			...
			_xhrFetch: function (s, e, t, n, r) {
				...
				for (o in t = function (e, t) {
					...
					i.onreadystatechange = function () {
						try {
							var e, t;
							4 === i.readyState && (e = i.response,
								t = 200 === i.status ? null : new Error("Error submitting data: " + (e ? e.error : "Unknown Error")),
								r(t, t ? null : e));
						} catch (t) {
							m._sendError(s, t),
								r(t);
			...
			_sendError: function (e, t) {
				var s = '{"error": "' + (t.stack || t.message || t + "").replace(/"/g, '\\"').replace(/\n/g, "\\n") + '"}'
					, t = m._xhr();
				t.open("POST", m._base + "/error/" + (e || "")),
					t.setRequestHeader("Content-Type", "application/json"),
					t.setRequestHeader("X-REQMETHOD", "send-v1"),
					t.send(s);
			},
			...
			_showSuccess: function (e, t) {
				...
				s.classList.add(m.cssBase + "__success"),
					s.innerHTML = t,
					s.onclick = function () {
						m._hideSuccess(e);
					}
	...
	function f(e) {
		...
		if (e.base !== m._base || !e.key.length)
			throw new Error(t);
		return e.url = e.base + "/" + e.key + "/" + e.bucket,
			e;
	}
	function p(e) {
	...
	p.prototype.start = function () {
		var e;
		this.el && (this.el.disabled = !0,
			(e = this.el.classList).remove(o),
			e.remove(r),
			e.add(n),
			i(this.el, function () {
				e.remove(n),
		...
		p.prototype.stop = function () {
			...
			var t;
			this.el && (this.el.disabled = !1,
				(t = this.el.classList).remove(n),
				t.remove(r),
				t.add(o),
				i(this.el, e, e));
		}
		;
	var c = null;
	"undefined" != typeof module && void 0 !== module.exports ? (module.exports = m,
		module.exports.normalizeAction = f) : (window.Pageclip = m,
			function () {
				for (var e = document.querySelectorAll("." + m.cssBase), t = 0; t < e.length; t++)
					m.form(e[t]);
			}());
}(this || window || global);
